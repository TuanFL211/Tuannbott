-- Gemini Chatbot Script for Roblox - Safe Version
...
local function CreateChatGUI()
    ...
    settingsBtn.Position = UDim2.new(1, -150, 0.5, -12.5)
    ...
    sendBtn.Position = UDim2.new(1, -50, 0.5, -20)
    ...
    settingsTitle.Text = "⚙ CÀI ĐẶT GEMINI AI"
    settingsTitle.TextColor3 = Color3.fromRGB(255, 255, 255)
    settingsTitle.TextSize = 16
    settingsTitle.Font = Enum.Font.GothamBold
    settingsTitle.Parent = settingsPanel

    -- API Key Input - QUAN TRỌNG: Người dùng phải tự nhập
    local apiKeyContainer = Instance.new("Frame")
    apiKeyContainer.Name = "ApiKeyContainer"
    apiKeyContainer.Size = UDim2.new(1, -20, 0, 80)
    apiKeyContainer.Position = UDim2.new(0, 10, 0, 50)
    ...
    apiKeyInput.PlaceholderText = "Dán API Key Gemini của bạn vào đây..."
    ...
    instructions.Position = UDim2.new(0, 0, 0, 65)
    instructions.BackgroundTransparency = 1
    instructions.Text = "Lấy API Key tại: makersuite.google.com/app/apikey"
    ...
    saveBtn.Position = UDim2.new(0, 10, 1, -50)
...
local function CallGeminiAPI(message)
    if CONFIG.API_KEY == "" or not CONFIG.API_KEY then
        return "❌ Vui lòng thiết lập API Key trong mục Cài Đặt trước khi sử dụng!"
    end
    ...
    local success, response = pcall(function()
        if syn and syn.request then
            ...
            local httpSuccess, httpResponse = pcall(function()
                return game:HttpGetAsync(url, true, HttpService:JSONEncode(requestBody))
            end)
            if httpSuccess then
                return httpResponse
            else
    ...
    if success and response then
        ...
        if decodeSuccess and data and data.candidates and data.candidates[1] and data.candidates[1].content then
            ...
            return "❌ Lỗi API: Vui lòng kiểm tra lại API Key!"
        end
    else
        return "❌ Lỗi kết nối: Không thể kết nối đến Gemini API"
    end
end
...
local function InitializeChat()
    ...
    -- Add welcome message
    AddMessage("🤖 Chào bạn! Để sử dụng Gemini AI, vui lòng:", false, true)
    AddMessage("1. Vào mục ⚙ Cài Đặt", false, true)
    AddMessage("2. Dán API Key Gemini của bạn", false, true)
    AddMessage("3. Nhấn 💾 LƯU API KEY", false, true)
    AddMessage("4. Đóng cửa sổ cài đặt và bắt đầu chat!", false, true)
    ...
    if saveBtn then
        saveBtn.MouseButton1Click:Connect(function()
            pcall(function()
                if apiKeyInput then
                    CONFIG.API_KEY = apiKeyInput.Text:gsub("^%s*(.-)%s*$", "%1")
                    if CONFIG.API_KEY ~= "" then
                        settingsPanel.Visible = false
                        AddMessage("✅ API Key đã được lưu! Bạn có thể bắt đầu chat.", false, true)
                    else
                        AddMessage("❌ Vui lòng nhập API Key trước khi lưu!", false, true)
                    end
                end
            end)
        end)
...
if not success then
    ...
    fallback.Name = "GeminiChatSimple"
    ...
    label.Text = "Gemini AI Chat\n\nVui lòng vào mục ⚙ Cài Đặt\nđể nhập API Key Gemini của bạn\n\nLấy API Key tại:\nmakersuite.google.com/app/apikey"
