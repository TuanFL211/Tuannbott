...
-- Hàm chuyển đổi đồ họa potato cực thấp
local function enablePotatoMode()
    local character = LocalPlayer.Character or LocalPlayer.CharacterAdded:Wait()
    
    -- Thiết lập Lighting tối giản
    ...
    -- Giảm chất lượng tất cả Part
    for _, obj in ipairs(Workspace:GetDescendants()) do
        pcall(function()
            if obj:IsA("Part") then
                -- Đặt chất lượng thấp nhất
    ...
    -- Xóa nước và hiệu ứng nước
    for _, obj in ipairs(Workspace:GetDescendants()) do
        if obj:IsA("Part") and (obj.Name:lower():find("water") or obj.Name:lower():find("wave")) then
            obj:Destroy()
        end
    end
    ...
    for _, sound in ipairs(SoundService:GetDescendants()) do
        if sound:IsA("Sound") and (sound.Name:match("Skill") or sound.Name:match("Effect")) then
            sound:Stop()
        end
    end
    
    -- Tắt các service tốn hiệu năng
...
potatoBtn.MouseButton1Click:Connect(function()
    if not isPotatoMode then
        ...
        StarterGui:SetCore("SendNotification", {
            Title = "POTATO MODE",
            Text = "Đồ họa đã được giảm xuống mức thấp nhất!",
            Duration = 3
        })
    else
        StarterGui:SetCore("SendNotification", {
...
StarterGui:SetCore("SendNotification", {
    ...
    Text = "Đã tải thành công! CRE: TUANN",
    Duration = 5
})
]])()